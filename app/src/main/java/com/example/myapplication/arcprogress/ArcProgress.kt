...
class ArcProgress @JvmOverloads constructor(
    ...
    protected fun initByAttributes(attributes: TypedArray) {
        ...
        thumbColor =
            attributes.getColor(R.styleable.ArcProgress_arc_thumb_color, default_finished_color)
        thumbSize =
            attributes.getDimension(R.styleable.ArcProgress_arc_thumb_size, default_stroke_width)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val scaledTop = rectF.centerY() - scaledHeight / 2
        val scaledRight = rectF.centerX() + scaledWidth / 2
        val scaledBottom = rectF.centerY() + scaledHeight / 2
        val scaledRectF = RectF(scaledLeft, scaledTop, scaledRight, scaledBottom)

        paint!!.color = unfinishedStrokeColor
        canvas.drawArc(scaledRectF, startAngle, arcAngle, false, paint!!)
        paint!!.color = finishedStrokeColor
        canvas.drawArc(scaledRectF, finishedStartAngle, finishedSweepAngle, false, paint!!)
...
//            arcBottomHeight = radius * (1 - Math.cos(angle / 180 * Math.PI)).toFloat()
    ...
    override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            thumbSize = bundle.getFloat(INSTANCE_THUMB_SIZE)
            thumbColor = bundle.getInt(INSTANCE_THUMB_COLOR)
            initPainters()
    ...
    companion object {
        ...
        private const val INSTANCE_THUMB_SIZE = "thumb_size"
        private const val INSTANCE_THUMB_COLOR = "thumb_color"
    }
}